  ...
  <div v-if="claim">
    <v-list-item two-line>
      <v-list-item-content>
        ...
        <v-list-item-subtitle v-if="past">
          {{ $moment(claim.claim_start).format("ll") }} &mdash;
          {{ $moment(claim.claim_end).format("ll") }}
        </v-list-item-subtitle>
        <v-list-item-subtitle
          v-else-if="future"
          :title="
            $moment(claim.claim_start).format('ll') +
              ' - ' +
              $moment(claim.claim_end).format('ll')
          "
        >
          <strong>
            {{ $moment(claim.claim_start).diff($moment(), "days") }} дней
          </strong>
          до начала брони
        </v-list-item-subtitle>
        <v-list-item-subtitle
    ...
    <locker-claim-dialog
      ref="edit"
      :claim="claim"
      title="Редактирование брони шкафчика"
      is-edit
    />
    <confirm ref="delete" />
  </div>
  ...
  methods: {
    ...
    updateClaim() {
      this.$refs.edit.open().then(form => {
        this.$axios
          .patch("locker-claims/" + this.claim.id, form)
          .then(async response => {
